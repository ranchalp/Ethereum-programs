...
let source = fs.readFileSync("chargingCompany.json");
...
let abi = JSON.parse(contracts[chargingContract].abi);let abi = JSON.parse(contracts[chargingContract].abi);
...
var caller = '0x2e4BF531EA8d0575BBfb2794d329a435426260B5';
var result;
...
//add Stations
var station1 = "0xac0814d45558b9055de432e6fa71dc04f7fa743a";
var station2 = "0x504386b046373f41149c33679e574aeab303c3ed";

console.log("adding first station...");
contract.methods.addStation(station1).send({from:caller}).then(function(_result){result = _result;});
contract.methods.addStation(station2).send({from:caller}).then(function(_result){result = _result;});

var user1 = "0x0c0fd5ad358680f71d3be56363fbe2f8f4b7b45a";
web3.eth.personal.unlockAccount(user1, 'test');
...
contract.methods.getTokenPrice().call()
	...
	{
	    tokenPrice = _result;
	    contract.methods.purchaseTokens(tokensToPurchase).send({from:user1,value:tokensToPurchase*tokenPrice}).then(function(_result){result = _result;
															       console.log(result)});
	});
