...
module.exports = {
    ...
    signAll: function(address, totalFuel,timestamp,station){
	var hash = web3.utils.soliditySha3(totalFuel,timestamp);//
	var sig_promise = web3.eth.sign(hash,address);
	//var result_station = personal.ecRecover(h, "0x"+sig)
	// send to user => value, station, result
	var result = [hash,sig_promise];
    ...
    rsvget_signer: function (hash, r,s,v) {
    ...
    verifyAll: function (totalFuel, timestamp, hash, address,signer_address) {
	var hashedData = web3.utils.soliditySha3(totalFuel,timestamp);//
	return (hash==hashedData) && (signer_address == address);
    }
};
